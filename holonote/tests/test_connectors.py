...
class TestConnector:
    ...
    def test_expand_range_region_column_schema_datetimes(self):
        result = Connector.expand_region_column_schema(
            ["Range"], {"xdim": np.datetime64, "ydim": int}
        )
        expected = {
    ...
    def test_generate_schema(self):
        ...
        result = Connector.generate_schema(
            AutoIncrementKey(), region_types, kdim_dtypes, {"description": str}
        )
        expected = {
...
class TestSQLiteDB:
    ...
    def test_add_row(self, database, request):
        ...
        df = pd.DataFrame(
            {
                "uuid": [database.primary_key.cast(id1)],
